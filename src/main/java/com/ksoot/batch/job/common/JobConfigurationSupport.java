...
import com.ksoot.batch.config.DataSourceConfig;
...
@Slf4j
@AutoConfigureAfter(value = {BatchConfiguration.class, DataSourceConfig.class})
...
//    private TaskExecutor taskExecutor;
    ...
    @Autowired
    private ObjectProvider<SkipListener<R, W>> skipListenerProvider;

    @Autowired
    private List<Class<? extends Throwable>> skippedExceptions;
...
//                        .taskExecutor(this.taskExecutor) // Multithreading in Step need verification if it works
                        .reader(reader)
                        .processor(processor)
                        .writer(writer)
                        .faultTolerant()
